...
REGION=${1:-us-east-1} # this means us-east-2 by default - can be us-east-1 if us-east-1 is passed as an argument
...
# ROBOT_API_CUSTOMERS
ROBOT_API_CUSTOMERS="SiteIQ,Sparc,university_of_florida,KBS"

# SNS Configuration - CHANGE THIS EMAIL TO YOUR ACTUAL EMAIL
NOTIFICATION_EMAIL="jiaxu.chen@siteiqtechnologies.com"  # ⚠️ CHANGE THIS TO YOUR EMAIL
...
# Prepare environment variables as JSON file to handle comma-separated values
cat > /tmp/lambda_env.json << EOF
{
  "Variables": {
    "SUPPORT_TICKET_SNS_TOPIC_ARN": "${SUPPORT_TOPIC_ARN}",
    ...
    "ICONS_CONFIG_PATH": "${ICONS_CONFIG_PATH}"
  }
}
EOF
