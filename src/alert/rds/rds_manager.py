...
class RDSManager:
    ...
    def get_alert_targets(self) -> List[Dict]:
        ...
        try:
            query = """
            SELECT
                cln_sensor_alarm_setting.*,
                pro_sensor_info.sensor_type
            FROM cln_sensor_alarm_setting
            JOIN pro_sensor_info
                ON pro_sensor_info.zone_id = cln_sensor_alarm_setting.zone_id
            JOIN pro_sensor_type_info
                ON pro_sensor_info.sensor_type = pro_sensor_type_info.sensor_type
                AND pro_sensor_info.data_type = pro_sensor_type_info.data_type
            JOIN cln_sensor_function_setting
                ON cln_sensor_alarm_setting.zone_id = cln_sensor_function_setting.zone_id
                AND cln_sensor_alarm_setting.data_type = cln_sensor_function_setting.data_type
            WHERE cln_sensor_function_setting.open_alarm = 1
            ORDER BY cln_sensor_alarm_setting.zone_id, cln_sensor_alarm_setting.data_type
            """
