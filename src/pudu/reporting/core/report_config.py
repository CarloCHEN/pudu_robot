...
class ReportConfig:
    ...
    def get_comparison_periods(self) -> Tuple[Tuple[str, str], Tuple[str, str]]:
        ...
        if self.time_range == 'custom' and self.custom_date_range:
            ...
            try:
                # Parse dates as naive datetime (no timezone info yet)
                if ' ' in current_start_str:
                    current_start_naive = datetime.strptime(current_start_str, '%Y-%m-%d %H:%M:%S')
                else:
                    # For date-only input, use start of day
                    current_start_naive = datetime.strptime(current_start_str, '%Y-%m-%d')
                    current_start_naive = current_start_naive.replace(hour=0, minute=0, second=0)

                if ' ' in current_end_str:
                    current_end_naive = datetime.strptime(current_end_str, '%Y-%m-%d %H:%M:%S')
                else:
                    # For date-only input, use end of day to include the full day
                    current_end_naive = datetime.strptime(current_end_str, '%Y-%m-%d')
                    current_end_naive = current_end_naive.replace(hour=23, minute=59, second=59)

                # Now localize to user's timezone (this marks them as being in that timezone)
                current_start = user_tz.localize(current_start_naive)
                current_end = user_tz.localize(current_end_naive)
        ...
        # Convert to UTC strings (the _convert_to_utc_string method handles the timezone conversion)
        current_period = (
