...
def handle_robot_status_notification(robot_sn: str, data: dict) -> Tuple[str, str, str, str]:
    ...
    if run_status == 'ONLINE':
        title = "Robot Online"
        content = f"Robot: {robot_sn}; Robot {robot_sn} is now online."
        severity = SEVERITY_LEVELS['success']
        status = STATUS_TAGS['online']

    elif run_status == 'OFFLINE':
        title = "Robot Offline"
        content = f"Robot: {robot_sn}; Robot {robot_sn} has gone offline."
        severity = SEVERITY_LEVELS['error']
        status = STATUS_TAGS['offline']

    elif run_status in ['WORKING', 'CLEANING']:
        title = "Robot Working"
        content = f"Robot: {robot_sn}; Robot {robot_sn} has started working."
        severity = SEVERITY_LEVELS['success']
        status = STATUS_TAGS['active']

    elif run_status == 'IDLE':
        title = "Robot Idle"
        content = f"Robot: {robot_sn}; Robot {robot_sn} is now idle."
        severity = SEVERITY_LEVELS['event']
        status = STATUS_TAGS['normal']

    elif run_status == 'CHARGING':
        title = "Robot Charging"
        content = f"Robot: {robot_sn}; Robot {robot_sn} is now charging."
        severity = SEVERITY_LEVELS['event']
        status = STATUS_TAGS['charging']

    elif run_status == 'ERROR':
        title = "Robot Error"
        content = f"Robot: {robot_sn}; Robot {robot_sn} is in error state."
        severity = SEVERITY_LEVELS['error']
        status = STATUS_TAGS['failed']

    else:
        title = "Robot Status Update"
        content = f"Robot: {robot_sn}; Robot {robot_sn} status changed to {run_status}."
        severity = SEVERITY_LEVELS['event']
        status = STATUS_TAGS['normal']
...
def handle_robot_power_notification(robot_sn: str, data: dict) -> Tuple[str, str, str, str]:
    ...
    if isinstance(power_level, (int, float)):
        if power_level < 5:
            ...
            content = f"Robot: {robot_sn}; Critical battery level at {power_level}%!"
            severity = SEVERITY_LEVELS['fatal']
            status = STATUS_TAGS['warning']

        elif power_level < 10:
            title = "Low Battery Alert"
            content = f"Robot: {robot_sn}; Low battery level at {power_level}%"
            severity = SEVERITY_LEVELS['error']
            status = STATUS_TAGS['warning']

        elif power_level < 20:
            title = "Battery Warning"
            content = f"Robot: {robot_sn}; Battery level at {power_level}%"
            severity = SEVERITY_LEVELS['warning']
            status = STATUS_TAGS['warning']

        elif power_level >= 95 and 'charg' in charge_state:
            title = "Battery Fully Charged"
            content = f"Robot: {robot_sn}; Battery fully charged at {power_level}%"
            severity = SEVERITY_LEVELS['success']
